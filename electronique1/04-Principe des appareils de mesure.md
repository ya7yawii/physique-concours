...
> [!note] Utilisation d'un oscilloscope : Mesures en mode XY
> Le mode XY permet de mesurer des fréquences en comparant le signal inconnu à un signal de fréquence étalon. Si le rapport des fréquences est rationnel, nous obtenons des courbes de Lissajous ([[#^figure8|fig. 8]]). Ce rapport est égal au rapport des nombres de points de contact entre la courbe avec les segments horizontaux et verticaux du rectangle circonscrit.
> Il permet aussi de mesurer les valeurs de crête des deux signaux ainsi que leur déphasage ([[#^figure9|fig. 9]]). Cependant ce déphasage n'est connu qu'au signe près ([[#^figure10|fig. 10]]) et généralement de façon très peu précise.
> Cette méthode est surtout adaptée à la mesure de faibles déphasages ou à la recherche d'une condition de déphasage nul.
> Remarque : À haute fréquence cette méthode ne peut être appliquée, car la bande passante de l'amplificateur horizontal est généralement plus faible que celle de l'amplificateur vertical. La conséquence est l'apparition d'un déphasage parasite à l'écran.

==**Oscilloscope numérique**== :
Du point de vue de son fonctionnement interne, l'oscilloscope numérique ressemble bien plus à un micro-ordinateur qu'à un oscilloscope analogique.
Les tensions $u_1$ et $u_2$, entrées sur les voies 1 et 2, sont transformées en une suite de valeurs numériques, puis traitées comme le ferait un ordinateur. Le résultat est affiché sous forme de courbes $u_1(t)$, $u_2(t)$ ou $u_1(u_2)$ sur un écran qui peut être un écran cathodique ou un écran plat à cristaux liquides.
Du point de vue de l'utilisateur, l'oscilloscope numérique a été conçu pour simuler les fonctions de l'oscilloscope analogique : base de temps, synchronisation, mode XY couplage AC ou DC, etc. Nous pouvons donc l'utiliser comme un appareil analogique, mais il possède en plus des fonctionnalités qui lui sont propres :
- il est possible d'afficher une courbe $u(t)$ non périodique, correspondant à un balayage unique de la base de temps. Il faut pour cela choisir le mode « monocoup » ou « single » ;
- on peut superposer plusieurs courbes obtenues successivement ;
- l'oscilloscope a également une fonction multimètre : on peut lui faire afficher la valeur de crête, la valeur moyenne et la valeur efficace des tensions mesurées. Il peut aussi indiquer la fréquence si le signal est périodique et le déphasage entre les deux tensions $u_1(t)$ et $u_2(t)$.

> [!note] Étude de la caractéristique d'un dipôle à l'oscilloscope
> Il faut tracer, en mode $XY$, le courant $i$ en fonction de la tension $u$. Nous devons pour cela résoudre deux problèmes.
> - L'oscilloscope est en fait un voltmètre. Pour avoir une image du courant $i$, nous allons devoir mesurer la tension aux bornes d'un résistor de résistance $R$ intercalé dans le circuit.
> - Les tensions $u_1$ et $u_2$ mesurées par l'oscilloscope sont mesurées entre deux bornes <mark style="color: red">dont l'une est à la masse</mark>. Or, la masse de l'oscilloscope est reliée à celle du générateur par le fil de terre de l'alimentation et il n'est pas possible de concevoir un circuit où le résistor, le dipôle et le générateur aient chacun une borne à la masse. Deux solutions sont envisageables.
> 
> Première solution : Étudions la [[#^figure11|figure 11]].
> La position de la masse est imposée par le générateur.
> La tension envoyée sur la voie 1 de l'oscilloscope est bien égale à la tension $u$ aux bornes du dipôle.
> La résistance $R$ n'ayant aucune borne à la masse, l'oscilloscope ne peut mesurer directement la tension à ses bornes. On y remédie avec une sonde différentielle.
> Cet appareil a deux bornes d'entrée ($A$ et $B$) et deux bornes de sortie ($C$ et $D$) dont l'une est à la masse.
> Les courants entrant par les bornes d'entrée $A$ et $B$ sont pratiquement nuls.
> La tension de sortie $u_C$ est égale à la différence de potentiel $u_A - u_B$ entre les deux bornes d'entrée.
> Finalement nous obtenons sur l'oscilloscope une courbe représentant $Ri$ en fonction de $u$.
> Seconde solution : Nous pouvons aussi utiliser une méthode moins coûteuse en matériel, mais approximative. Inversons le branchement du générateur et choisissons la résistance $R$ suffisamment faible pour que la tension $u_R = Ri$ soit petite devant la tension $u$.
> Avec une assez bonne approximation : $u \approx u + u_R = -u_g$.
> Nous voyons sur la [[#^figure12|figure 12]] que la voie 2 mesure la tension $u_R = Ri$ et la voie 1 la tension $u_g \approx -u$.
# Diagrammes
...
Utilisation d'une sonde différentielle
![[electronique1/attachments-electronique1/figure72.png]]^figure11

Tracé approximatif de la caractéristique du dipôle
![[electronique1/attachments-electronique1/figure73.png]]^figure12
# Graphiques
...
![[electronique1/attachments-electronique1/figure69.png]]^figure8

Grandeurs mesurables en mode $XY$ avec : $u_x = u_{x_m}\cos(\omega t)$, $u_y = u_{y_m}\cos(\omega t + \varphi)$ 
![[electronique1/attachments-electronique1/figure70.png]]^figure9

Déphasage en fonction de l'orientation de l'ellipse pour $u_x = u_{x_m}\cos(\omega t)$ et $u_y = u_{y_m}\cos(\omega t + \varphi)$
![[electronique1/attachments-electronique1/figure71.png]]^figure10
# Expériences
> [!warning]
> Voici des exemples de travaux pratiques qui abordent le sujet de ce chapitre : le [lien 1](https://www.technologuepro.com/mesures-electriques-l1/tp2-utilisation-de-oscilloscope.pdf), le [lien 2](https://staff.univ-batna2.dz/sites/default/files/amadji-moussa/files/tp-oscilloscope.pdf), le [lien 3](https://webetab.ac-bordeaux.fr/Etablissement/BDBorn/sections/postbac/prepasciences/physique/telech/docs20089/TP11-12_2008-2009.pdf), le [lien 4](http://purple.univ-littoral.fr/etudes/TPL1/TP1-Oscilloscope_et_regime_continu.pdf), le [lien 5](https://perso.univ-lemans.fr/~nerrien/Correction%20TP%20S2/CorrigesElectroTP1.pdf), le [lien 6](https://ressources.unisciel.fr/sillages/physique/tp_electrocinetique_1a_pcsi/res/TP2.PDF), le [lien 7](http://cdelacourphysique.chez.com/TP/Manipulations%20oscilloscope%20numerique.pdf), le [lien 8](https://prepanouar.wordpress.com/wp-content/uploads/2020/03/oscilloscope.pdf), le [lien 9](https://fr.scribd.com/document/833270921/TP-cours-Instrument-de-mesure-en-electronique-oscilloscope), le [lien 10](https://sc-st.univ-batna2.dz/sites/default/files/sc_st/files/oscilloscope_cathodique.pdf), le [lien 11](https://elearning.enp-oran.dz/moodle/pluginfile.php/12871/mod_resource/content/7/TP4%20OscilloTuto.pdf), le [lien 12](https://staff.univ-batna2.dz/sites/default/files/abdessemed_mustapha/files/tp3_oscilloscope.pdf), le [lien 13](http://ghostyd.free.fr/ressources_spe_phys_term/C0%20logiciels%20ece/TP%20oscillo.pdf), le [lien 14](http://remy.duperray.free.fr/page5/files/TP-Oscilloscope.pdf), le [lien 15](https://www.studocu.com/row/document/ecole-nationale-superieure-de-technologie-alger/physique-statique/tp-physique-n4-loscilloscope/58336882), le [lien 16](https://docs.google.com/viewerng/viewer?url=dzuniv.com/storage/media/df7f7696254998ad5ef33265897d865b/tp_physique_n4_loscilloscope.pdf), le [lien 17](https://elearn.univ-tlemcen.dz/pluginfile.php/112276/mod_resource/content/1/TP5_Oscilloscope_2020.pdf).
# Autres notes
