...
> [!note] Résolution d'un réseau par équivalences successives : Associations de résistances et ponts diviseurs
> Cherchons les intensités des courants dans le circuit représenté sur la [[#^figure12|figure 12]]. Pour cela, nous allons calculer $i_0$ puis en déduire les autres par divisions de courant.
...
> $i_0$ se calcule à partir du circuit équivalent à maille unique ([[#^figure14|fig. 14]]) : $i_0 = \frac{e}{3R}$.
> Pour déterminer $i_1$, nous raisonnons sur la [[#^figure15a|figure 15a]] où l'on a fait figurer la résistance équivalente $R_{BB'} = 2R$. La relation du diviseur de courant nous donne : $i_1 = i_0\frac{\frac{1}{6R}}{\frac{1}{6R} + \frac{1}{3R}} = \frac{1}{3}i_0$ et $i_{AB} = \frac{2}{3}i_0$. Nous déterminons aussi $i_2$ par division du courant $i_{AB}$ au nœud $B$ ([[#^figure15b|fig. 15b]]). Nous obtenons : $i_2 = \frac{1}{3}i_{AB}$ et $i_{BC} = \frac{2}{3}i_{AB}$. Un calcul identique au noeud $C$ nous donne enfin : $i_3 = \frac{1}{3}i_{BC}$ et $i_{4} = \frac{2}{3}i_{BC}$.
> Remarque : Nous aurions pu aussi calculer les tensions $u_{AA'}$, $u_{BB'}$ et $u_{CC'}$ par divisions successives de tensions.
> De cette étude, nous retenons que :
> - Pour calculer la résistance vue par la source, il faut commencer par les résistances qui en sont le plus éloignées.
> - Une fois connu le courant débité par la source, on détermine les courants (ou les tensions) en appliquant la formule du diviseur de courant (ou du diviseur de tension).

> [!note] Résolution d'un réseau par équivalences successives : Utilisation successive des représentations de Norton et de Thévenin
> Considérons à nouveau le circuit étudié dans l'Application 6, représenté sur la [[#^figure6|figure 6]].
> La [[#^figure16|figure 16]] montre comment, en transformant successivement des générateurs de Thévenin en générateur de Norton (ou l'inverse), on se ramène à une maille unique.
> En appliquant la formule du diviseur de tension sur le dernier schéma de la [[#^figure16|figure 16]], nous obtenons : $u = -\left(\frac{E}{2} + 3\eta R\right)\frac{R}{5R}$.
> De cette étude nous retenons que :
> - lorsque deux générateurs sont en série, il faut les mettre sous la forme de générateur de Thévenin ;
> - lorsque deux générateurs sont en parallèle, il faut les mettre sous la forme de générateurs de Norton ;
> - le but est de se ramener à une maille unique avec des générateurs de Thévenin (ou à un circuit à deux nœuds avec des générateurs de Norton et des résistors tous en parallèle).
# Définitions
...
Circuit équivalent pour le calcul de $i_1$
![[electronique1/attachments-electronique1/figure54.png]]^figure15a

Circuit équivalent pour le calcul de $i_2$
![[electronique1/attachments-electronique1/figure55.png]]^figure15b

Utilisation successive des représentations de Norton et Thévenin
![[electronique1/attachments-electronique1/figure56.png]]^figure16
# Graphiques

# Expériences
> [!warning]
> Voici des exemples de travaux pratiques qui abordent le sujet de ce chapitre : le [lien 1](https://fss.rnu.tn/useruploads/files/D%C3%A9partement%20Physique/2025-2026/Fascicule%20BG1%20final%2025-26.pdf), le [lien 2](https://fr.scribd.com/document/728402467/TP-N-2-Theoreme-de-superposition).
# Autres notes
> [!warning]
