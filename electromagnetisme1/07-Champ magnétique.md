...
> Poursuivant l'étude précédente, nous pouvons envisager un circuit obtenu par enroulement régulier d'un fil conducteur sur un cylindre d'axe $(Oz)$, les $N$ tours de fil de même rayon $R$ occupant une longueur totale $l$. En pratique, le nombre $n$ de tours de fil par unité de longueur est élevé, et nous pouvons l'assimiler à un ensemble de spires d'axe $(Oz)$ quasi jointives et de rayon $R$.
> Le circuit obtenu est un solénoïde (du grec solên : étui, tuyau) à section circulaire et à une couche (nous pourrions envisager plusieurs épaisseurs d'enroulement).
> Observons les [[#^figure24a|figures 24 a]] et [[#^figure24b|b]], qui présente quelques lignes de champ d'un solénoïde (à $N = 21$ spires très proches) dans un plan contenant $(Oz)$ :
> - le solénoïde canalise les lignes de champ magnétique ;
> - les lignes de champ s'écartent tellement vite dès leur sortie du solénoïde que nous pouvons prévoir une atténuation très rapide du champ à l'extérieur du solénoïde, où il doit être négligeable devant le champ à l'intérieur du solénoïde.

> [!note] Flux du champ magnétique : Un exemple de canalisation du flux magnétique ; le solénoïde
> - Champ sur l'axe du solénoïde :
> Tout plan contenant l'axe du solénoïde est un plan d'antisymétrie, donc sur l'axe le champ est de la forme $\overrightarrow{B}_{axe} = B_{axe}(z)\overrightarrow{e}_z$. Le champ créé en un point $M$ de cote $z_M$ de l'axe par une spire de cote $z_P = z_M + R\,\text{cotan}\,\alpha$ parcourue par le courant $I$ est $\frac{\mu_0I}{2R}\sin^{3}\alpha$ ([[#^figure25a|fig. 25 a]]).
> Celui créé par les spires de cote comprise entre $z_P$ et $z_P + dz_P$ $\left(dz_P = -\frac{Rd\alpha}{\sin^{2}\alpha}\right)$, au nombre de $n\,dz_P$, vaut : $\frac{\mu_0\,n\,Idz_P}{2R}\sin^{3}\alpha = -\frac{\mu_0\,n\,I}{2}\sin\alpha\,d\alpha = \frac{\mu_0\,n\,I}{2}d(\cos\alpha)$.
> Notant $\alpha_1$ et $\alpha_2$ les angles extrêmes (compris entre $0$ et $\pi$) sous lesquels les extrémités du solénoïdes sont vues depuis le point $M$ ([[#^figure25b|fig. 25 b]]), nous obtenons le champ magnétique total sur l'axe du solénoïde : $\overrightarrow{B}_{axe} = \mu_0\,n\,I\left(\frac{\cos\alpha_2 - \cos\alpha_1}{2}\right)\overrightarrow{e}_z$.
> Le tracé de la valeur du champ sur l'axe $(z'z)$ en fonction de $z$ ([[#^figure26|fig. 26]]) montre que le champ magnétique est pratiquement uniforme à l'intérieur du solénoïde et devient rapidement négligeable à l'extérieur.
> - Limite du solénoïde infini :
> Pour un solénoïde très long, c'est-à-dire lorsque le rapport $\frac{l}{R}$ est très grand (idéalement infini), $\alpha_1$ tend vers $\pi$ et $\alpha_2$ tend vers $0$.
> ==**Le champ magnétique sur l'axe d'un solénoïde infiniment long uniforme et comportant $n$ spires par unité de longueur vaut : $\overrightarrow{B}_{\infty} = \mu_0\,n\,I\,\overrightarrow{e}_z$.**==
> Nous verrons au chapitre 8, que lorsque la limite $\frac{l}{R}$ tend vers $\infty$, le champ est uniforme, égal à $\mu_0\,n\,I\,\overrightarrow{e}_z$, en tout point à l'intérieur du solénoïde, et il est nul à l'extérieur.
# Définitions
...
![[electromagnetisme1/attachments-electromagnetisme1/figure132.png]]^figure25a

![[electromagnetisme1/attachments-electromagnetisme1/figure133.png]]^figure25b
# Graphiques
...
Lignes de champ magnétique d'un ensemble de vingt et une spires régulièrement réparties et parcourues par des courants identiques
![[electromagnetisme1/attachments-electromagnetisme1/figure130.png]]^figure24a

Allure de $B(z)$ pour un solénoïde constitué de vingt et une spires régulièrement réparties et parcourues par des courants identiques
![[electromagnetisme1/attachments-electromagnetisme1/figure131.png]]^figure24b

Solénoïde aux extrémités d'abscisses $-z_0$ et $z_0$
![[electromagnetisme1/attachments-electromagnetisme1/figure134.png]]^figure26
# Expériences
> [!warning]
> Voici des exemples de travaux pratiques qui abordent le sujet de ce chapitre : le [lien 1](https://www.fresnel.fr/perso/stout/electromag/TP_Enonce_EM.pdf), le [lien 2](https://tpmpathome.univ-lille.fr/force_laplace_iut/version_papier/_papier.pdf), le [lien 3](https://ressources.unisciel.fr/tp_virtuels/champs_magnetiques_serveur_IUT/champs%20magnetiques_papier_QRcode.pdf), le [lien 4](https://elearning.esgee-oran.dz/pluginfile.php/16193/mod_page/content/68/TRAUVAUX%20PRATIQUES%20DE%20LA%20PHYSIQUE.pdf), le [lien 5](https://www.physagreg.fr/tp/TP5_magnetisme.pdf), le [lien 6](http://physiqueensti.free.fr/IMG/pdf/1sti/1sti_TP09.pdf), le [lien 7](http://lefevre.pc.free.fr/site_3/download/pdf/premieres/tpphysique/1stpp16.pdf), le [lien 8](https://www.fresnel.fr/perso/fehrembach/Enseignement/TPElecEtMag.pdf), le [lien 9](https://fr.scribd.com/document/714470179/Poly-TP), le [lien 10](http://remy.duperray.free.fr/downloads-4/files/TP-Champ-Magnetique.pdf), le [lien 11](https://fr.scribd.com/document/493939609/TP-1-ETT), le [lien 12](https://www.cpge-paradise.com/pdf/TP-magnetostatique.pdf), le [lien 13](http://physik.diekirch.org/2e/TP12%20ChampBobine16.pdf), le [lien 14](https://thesis.essa-tlemcen.dz/bitstream/handle/STDB_UNAM/398/TP%20N%20%E2%97%A6%206%20%20Champ%20magn%C3%A9tique%20autour%20d%E2%80%99un%20fil%20conducteur..pdf?sequence=9&isAllowed=y), le [lien 15](http://www.mmelzani.fr/documents/2025/induction/TP_22_mesure_BTerre.pdf), le [lien 16](http://purple.univ-littoral.fr/etudes/TPL2UO/5_Magnetostatique_L2S3_DunkerqueCalais_20181114.pdf), le [lien 17](https://fr.scribd.com/document/508349603/TP-dvi), le [lien 18](https://fr.scribd.com/document/854380301/tp-magnetique), le [lien 19](http://physiqueensti.free.fr/docs/1gm/tp05_champ_magnetique.pdf), le [lien 20](https://www.studocu.com/row/document/institut-national-de-statistique-et-deconomie-appliquee/urbanisation-des-si/tp-1-champ-du-solenoide-eefnkme/59926146), le [lien 21](https://www.phycats.plaf.org/docs/E3tp_modele_solenoide_infini.pdf), le [lien 22](https://fr.scribd.com/document/828390221/Solenoide-tp).
# Autres notes
