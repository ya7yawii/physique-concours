...
> - Principe du générateur de fonction : Un générateur de fonction est un circuit délivrant des signaux contrôlables en fréquence, dont les deux signaux de base sont :
> 	- les signaux carrés de rapport cyclique $\delta = 0,5$ ;
> 	- les signaux triangulaires symétriques.
> A l'aide de composants réglables, il est ensuite possible de modifier les signaux précédents en :
> 	- signaux carrés de rapport cyclique $\delta \neq 0,5$ ;
> 	- signaux triangulaires non symétriques.
> 
> 	Par utilisation d'un conformateur à diode ([[#^figure30|fig. 30]] cf. exercice 11 page 140), les signaux triangulaires sont transformés en signaux quasi sinusoïdaux de grande amplitude.
> 	Un générateur de fonctions est généralement constitué d'une boucle comprenant un comparateur à hystérésis et un intégrateur.
> 	Il est conçu de telle sorte que le point de fonctionnement du comparateur se déplace toujours vers ses points de basculement B et B' (comme pour un astable).
> 	Il est possible d'utiliser les deux types de comparateurs à condition de lui associer le type d'intégrateur adéquat : pour un comparateur inverseur ([[#^figure31|fig. 31]]) et pour un comparateur non inverseur ([[#^figure32|fig. 32]]).
> - Réalisation d'un générateur de fonctions à comparateur non inverseur à hystérésis : Tout ce qui est donné dans ce chapitre est le même que celui résumé dans le chapitre [[10-Amplificateur opérationnel. Bande passante, stabilité des montages bouclés et comparateurs|10]] de l'ouvrage H-Prépa, Électronique-Électrocinétique, 1re année.
> - Calcul de la période : Tout ce qui est donné dans ce chapitre est presque le même que celui résumé dans le chapitre [[10-Amplificateur opérationnel. Bande passante, stabilité des montages bouclés et comparateurs|10]] de l'ouvrage H-Prépa, Électronique-Électrocinétique, 1re année.
> - Contrôle de la période et du rapport cyclique : Tout ce qui est donné dans ce chapitre est le même que celui résumé dans le chapitre [[10-Amplificateur opérationnel. Bande passante, stabilité des montages bouclés et comparateurs|10]] de l'ouvrage H-Prépa, Électronique-Électrocinétique, 1re année.
# Diagrammes
Quadripôle passif
...
![[electronique2/attachments-electronique2/figure75.png]]^figure30

Schéma fonctionnel d'un astable réalisé avec un comparateur inverseur
![[electronique2/attachments-electronique2/figure76.png]]^figure31

Schéma fonctionnel d'un générateur de signaux réalisé avec un comparateur non inverseur
![[electronique2/attachments-electronique2/figure77.png]]^figure32
# Graphiques
...
# Expériences
 > [!warning]
> La plupart des exemples de travaux pratiques sur l'amplificateur opérationnel et ses différents montages se trouvent dans les chapitres [[07-L'amplificateur opérationnel. Le modèle idéal|7]] et [[10-Amplificateur opérationnel. Bande passante, stabilité des montages bouclés et comparateurs|10]].
# Autres notes
